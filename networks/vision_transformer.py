...
class SwinUnet(nn.Module):
    ...
    def load_from(self, config):
        ...
        if not os.path.isfile(pretrained_path):
            print("pretrained checkpoint not found: {}".format(pretrained_path))
            return
        ...
        if "model" not in pretrained_dict:
            ...
            for k in list(pretrained_dict.keys()):
                if k not in model_dict:
                    ...
                    continue
                if "output" in k or pretrained_dict[k].shape != model_dict[k].shape:
                    print(
                        "skip key:{};shape pretrain:{};shape model:{}".format(
